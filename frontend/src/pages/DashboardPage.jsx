...
const orangeS = ["#fb923c","#f97316","#ea580c","#ea580c","#ea580c"];
    ...
    <div className="stat-card animate-scale-in">
      <div className="flex items-start justify-between">
        <div>
          ...
          {sub && <p className="text-xs text-orange-400 mt-0.5">{sub}</p>}
        </div>
...
const orangeTooltip = ({ active, payload, label }) => {
      ...
      {showFilters && (
        <div className="glass-card p-4 animate-slide-up">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowFilters(false)} className="text-orange-400 hover:text-orange-600"><MdClose /></button>
          </div>
          ...
          {loading ? <SkeletonChart /> : stats?.monthly_trends?.length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={stats.monthly_trends}>
                ...
                <XAxis dataKey="month" tick={{ fontSize: 9, fill: "#fb923c" }} />
                <YAxis tick={{ fontSize: 9, fill: "#fb923c" }} width={40} />
                <Tooltip content={<orangeTooltip />} />
                <Area type="monotone" dataKey="sales" stroke="#f97316" strokeWidth={2} fill="url(#sg)" dot={{ r: 2 }} />
              </AreaChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={stats.model_breakdown} layout="vertical">
                ...
                <Bar dataKey="avg_accuracy" radius={[0,4,4,0]}>
                  {stats.model_breakdown.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={stats.top_products.slice(0,5)} layout="vertical">
                ...
                <Bar dataKey="sales" radius={[0,4,4,0]}>
                  {stats.top_products.slice(0,5).map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={stats.region_breakdown} dataKey="sales" nameKey="region" cx="50%" cy="50%" outerRadius={60}
                  label={({ region }) => region}>
                  {stats.region_breakdown.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-4">
        <div className="glass-card p-4 md:p-5">
          <h2 className="section-title mb-3 md:mb-4 flex items-center gap-2">
            <MdAccessTime className="text-orange-500" /> Recent Activity
          </h2>
          {activity.length === 0 ? (
            ...
            <div className="space-y-2">
              {activity.map((a) => (
                <div key={a.id} className="flex items-center justify-between py-2 border-b border-orange-100 dark:border-orange-900 gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: "var(--text)" }}>{a.name}</p>
                    <p className="text-xs font-mono text-orange-400">{a.model}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <StatusBadge status={a.status} />
                    {a.accuracy && <p className="text-xs text-orange-600 mt-0.5">{a.accuracy}%</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {loading ? <SkeletonChart /> : stats?.recent_forecasts?.length > 0 ? (
            <div className="space-y-2">
              {stats.recent_forecasts.map((f) => (
                <div key={f.id} className="flex items-center justify-between py-2 border-b border-orange-100 dark:border-orange-900 gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate" style={{ color: "var(--text)" }}>{f.name}</p>
                    <p className="text-xs font-mono text-orange-400">{f.model}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <StatusBadge status={f.status} />
                    {f.accuracy && <p className="text-xs text-orange-600 mt-0.5">{f.accuracy}%</p>}
                  </div>
                </div>
              ))}
            </div>
