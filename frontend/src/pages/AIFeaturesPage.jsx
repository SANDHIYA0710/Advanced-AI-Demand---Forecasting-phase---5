  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        <h1 className="page-title flex items-center gap-2"><TbBrain className="text-orange-500" /> Advanced AI Features</h1>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[["Total Products", data.total_products], ["High Priority", data.high_priority_count], ["Overall Trend", data.overall_trend], ["Growth Rate", `${data.growth_rate_pct}%`]].map(([label, val]) => (
          <div key={label} className="stat-card"><p className="text-xs text-orange-500 uppercase">{label}</p><p className="font-display text-xl font-bold mt-1" style={{ color:"var(--text)" }}>{val}</p></div>
        ))}
      </div>
      <div className="glass-card p-4 md:p-5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.recommendations?.map((r, i) => (
                <tr key={i} className="border-b border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30">
                  ...
                  <td className="table-td text-xs text-orange-600 dark:text-orange-400">{r.recommendation}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={dowData}>
            ...
            <Bar dataKey="value" radius={[4,4,0,0]}>
              {dowData.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="stat-card"><p className="text-xs text-orange-500 uppercase">Spikes Predicted</p><p className="font-display text-2xl font-bold mt-1" style={{ color:"var(--text)" }}>{data.spike_count}</p></div>
        <div className="stat-card"><p className="text-xs text-orange-500 uppercase">Spike Threshold</p><p className="font-display text-2xl font-bold mt-1" style={{ color:"var(--text)" }}>{data.spike_threshold}</p></div>
      </div>
      <div className="glass-card p-4 md:p-5">
        <p className="text-sm text-orange-600 dark:text-orange-400 mb-4">{data.recommendation}</p>
        {data.spikes?.length > 0 && (
          <div className="space-y-2">
            {data.spikes.map((s, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2">
                <div>
                  <p className="font-mono text-xs text-orange-500">{s.date}</p>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[["Risk Level", <span key="r" className={riskColors[data.risk_level]}>{data.risk_level?.toUpperCase()}</span>],
          ...
          <div key={label} className="stat-card"><p className="text-xs text-orange-500 uppercase">{label}</p><p className="font-display text-lg font-bold mt-1" style={{ color:"var(--text)" }}>{val}</p></div>
        ))}
      </div>
      <div className="glass-card p-4 md:p-5">
        <h2 className="section-title mb-3">Recommendation</h2>
        <p className="text-sm text-orange-700 dark:text-orange-300 bg-primary-50 dark:bg-primary-900/40 p-3 rounded-xl">{data.recommendation}</p>
        {data.reorder_date && <p className="text-xs text-orange-500 mt-2">Reorder by: <strong>{data.reorder_date}</strong></p>}
      </div>
    </div>
  );
}
        ...
        <div className="space-y-2">
          {data.suggestions?.map((s, i) => (
            <div key={i} className="flex items-start gap-2 p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl">
              ...
              <p className="text-sm text-orange-700 dark:text-orange-300">{s}</p>
            </div>
          ))}
        </div>
      </div>
