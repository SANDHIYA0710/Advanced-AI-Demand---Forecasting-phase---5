  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        <h1 className="page-title flex items-center gap-2"><MdPeople className="text-orange-500" /> Collaboration</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="md:col-span-1 glass-card p-4">
          ...
          {loading ? <SkeletonTable rows={3} /> : (
            <div className="space-y-2 max-h-80 overflow-y-auto scrollbar-thin">
              {forecasts.length === 0 ? (
                <p className="text-xs text-orange-400 text-center py-4">No completed forecasts.</p>
              ) : forecasts.map((f) => (
                <button key={f.id} onClick={() => handleSelectForecast(f)}
                  className={`w-full text-left p-3 rounded-xl transition-all text-xs ${selectedForecast?.id === f.id ? "bg-orange-500 text-white" : "bg-primary-50 dark:bg-primary-900/40 hover:bg-primary-100 dark:hover:bg-primary-900"}`}>
                  <p className="font-medium truncate" style={{ color: selectedForecast?.id === f.id ? "white" : "var(--text)" }}>{f.name}</p>
                  <p className={`mt-0.5 font-mono ${selectedForecast?.id === f.id ? "text-orange-200" : "text-orange-400"}`}>{f.model_type}</p>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="md:col-span-3 glass-card p-4 md:p-5">
          {!selectedForecast ? (
            <div className="h-48 flex flex-col items-center justify-center text-orange-300">
            ...
            <>
              ...
              {tab === "comments" && (
                <div className="space-y-4">
                  <div className="max-h-64 overflow-y-auto scrollbar-thin space-y-3 pr-1">
                    {comments.length === 0 ? (
                      <p className="text-sm text-orange-400 text-center py-6">No comments yet. Start the conversation!</p>
                    ) : comments.map((c) => (
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-xs font-semibold text-orange-600">{c.is_mine ? "You" : `User #${c.user_id}`}</span>
                                {c.is_edited && <span className="text-[10px] text-orange-400">(edited)</span>}
                              ...
                              {editingComment === c.id ? (
                                <div className="flex gap-2 mt-1">
                                  ...
                                  <button onClick={() => setEditingComment(null)} className="text-orange-400"><MdClose /></button>
                                </div>
                              ) : <p className="text-sm" style={{ color:"var(--text)" }}>{c.content}</p>}
                              <p className="text-[10px] text-orange-400 mt-1">{c.created_at?.slice(0,16).replace("T"," ")}</p>
                            </div>
                            {c.is_mine && editingComment !== c.id && (
                              <div className="flex gap-1 flex-shrink-0">
                                <button onClick={() => { setEditingComment(c.id); setEditContent(c.content); }} className="text-orange-400 hover:text-orange-600"><MdEdit className="text-sm" /></button>
                                <button onClick={() => handleDeleteComment(c.id)} className="text-red-400 hover:text-red-600"><MdDelete className="text-sm" /></button>
                              </div>
                            )}
                          </div>
                          {!c.is_mine && <button onClick={() => setReplyTo(c.id === replyTo ? null : c.id)} className="text-xs text-orange-500 hover:text-orange-700 mt-1">↩ Reply</button>}
                        </div>
                        {c.replies?.map((r) => (
                          <div key={r.id} className={`ml-6 p-2.5 rounded-xl text-xs ${r.is_mine ? "bg-primary-100 dark:bg-primary-800" : "bg-primary-50 dark:bg-primary-900/40"}`}>
                            <span className="font-semibold text-orange-600">{r.is_mine ? "You" : `User #${r.user_id}`}: </span>
                  ...
                  {replyTo && <p className="text-xs text-orange-500 bg-primary-50 dark:bg-primary-900/40 px-3 py-1.5 rounded-lg">Replying to comment #{replyTo} — <button onClick={() => setReplyTo(null)} className="text-red-400">Cancel</button></p>}
              ...
              {tab === "revisions" && (
                <div className="space-y-2">
                  {revisions.length === 0 ? (
                    <p className="text-sm text-orange-400 text-center py-6">No revision history yet.</p>
                  ) : revisions.map((r) => (
                    <div key={r.id} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl">
                      <div>
                        ...
                        <p className="text-xs text-orange-400">{r.model_type} • {r.changes || "Forecast updated"}</p>
                        <p className="text-xs text-orange-400">{r.created_at?.slice(0,16).replace("T"," ")}</p>
                      </div>
              ...
              {tab === "sharing" && (
                <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        {shares.map((s) => (
                          <div key={s.id} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2">
                            <div>
                              <p className="text-xs font-mono text-orange-600">{s.share_token}</p>
                              <p className="text-xs text-orange-400">{s.view_count} views • Expires: {s.expires_at?.slice(0,10)}</p>
                            </div>
                  ...
                  {shares.length === 0 && <p className="text-sm text-orange-400 text-center py-4">No share links created yet.</p>}
                </div>
              )}
            </>
          )}
