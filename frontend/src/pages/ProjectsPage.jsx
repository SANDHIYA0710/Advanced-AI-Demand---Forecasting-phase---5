      ...
      {showForm && (
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2"><MdFolder className="text-orange-500" /> New Project</h2>
      ...
      <div className="flex gap-2">
        {["active","archived"].map((s) => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`px-4 py-2 rounded-xl text-xs font-medium transition-all capitalize ${filterStatus === s ? "bg-orange-500 text-white" : "btn-secondary"}`}>
            {s}
          </button>
        ))}
      </div>
        ...
        <div className="md:col-span-1 space-y-3">
          {loading ? <SkeletonTable rows={3} /> : projects.length === 0 ? (
            <div className="glass-card p-8 text-center">
              <MdFolderOpen className="text-4xl text-orange-200 mx-auto mb-3" />
              <p className="text-sm" style={{ color:"var(--text-muted)" }}>No {filterStatus} projects yet.</p>
            </div>
          ) : projects.map((p) => (
            <div key={p.id} onClick={() => handleSelectProject(p)}
              className={`glass-card p-4 cursor-pointer transition-all hover:border-orange-400 ${selected?.id === p.id ? "border-orange-400 shadow-orange-glow" : ""}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <MdFolder className="text-orange-500 flex-shrink-0" />
                    <p className="font-semibold text-sm truncate" style={{ color:"var(--text)" }}>{p.name}</p>
                  </div>
                  {p.description && <p className="text-xs text-orange-400 truncate mb-2">{p.description}</p>}
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {p.tags?.map((t) => <span key={t} className="badge bg-primary-100 dark:bg-primary-900 text-orange-600 dark:text-orange-400">{t}</span>)}
                  </div>
                </div>
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={(e) => { e.stopPropagation(); handleArchive(p.id); }} className="text-orange-400 hover:text-orange-600 p-1" title={p.status === "active" ? "Archive" : "Unarchive"}>
              ...
              <div className="flex gap-3 mt-2 text-xs text-orange-400">
        ...
        <div className="md:col-span-2 glass-card p-4 md:p-5">
          {selected ? (
            <div>
              ...
              {activity.length === 0 ? (
                <p className="text-sm text-orange-400 text-center py-8">No activity yet for this project.</p>
              ) : (
                  ...
                  {activity.map((a) => (
                    <div key={a.id} className="flex items-start gap-3 p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-orange-400">{a.created_at?.slice(0,16).replace("T"," ")}</p>
                      </div>
                    </div>
                  ))}
                </div>
