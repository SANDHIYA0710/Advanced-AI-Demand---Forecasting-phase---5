    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex items-start justify-between gap-3 pt-2 md:pt-0">
        <div>
          ...
          <p className="text-orange-500 text-xs md:text-sm mt-1">AI-powered demand predictions</p>
        </div>
        ...
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2">
              <TbBrain className="text-orange-500" /> Configure Forecast
            </h2>
            <button onClick={() => setShowForm(false)} className="text-orange-400 hover:text-orange-600">
              <MdClose />
            </button>
          </div>
      ...
      {showCompare && (
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2">
              <MdCompare className="text-orange-500" /> Compare All Models
            </h2>
            <button onClick={() => { setShowCompare(false); setCompareResult(null); }}
              className="text-orange-400 hover:text-orange-600"><MdClose /></button>
          </div>
          ...
          {compareResult && (
            <div className="mt-4 pt-4 border-t border-orange-100">
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                <span className="text-sm font-semibold text-orange-800">Best Model:</span>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {compareResult.results.map((r) => (
                      <tr key={r.model} className={`border-b border-orange-50 ${r.model === compareResult.best_model ? "bg-primary-50" : ""}`}>
                        <td className="table-td font-mono text-xs">{r.model} {r.model === compareResult.best_model && "⭐"}</td>
                        <td className="table-td font-bold text-orange-700">{r.r2_score ? `${r.r2_score}%` : "—"}</td>
        ...
        <div className="md:hidden fixed inset-0 bg-black/40 z-50 flex items-end"
          ...
          <div className="bg-white w-full rounded-t-2xl p-4 max-h-[80vh] overflow-y-auto"
            ...
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-orange-900 text-sm truncate pr-2">{selected.name}</h3>
              <button onClick={() => setShowChart(false)} className="text-orange-400"><MdClose /></button>
            </div>
            <div className="flex gap-2 flex-wrap mb-3">
              {selected.accuracy_score && <span className="badge-success text-xs">R²: {(selected.accuracy_score * 100).toFixed(1)}%</span>}
              {selected.mae && <span className="badge text-xs bg-primary-50 text-orange-700">MAE: {selected.mae.toFixed(3)}</span>}
              {selected.rmse && <span className="badge text-xs bg-primary-50 text-orange-700">RMSE: {selected.rmse.toFixed(3)}</span>}
            </div>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="historical" stroke="#ea580c" strokeWidth={2} dot={false} name="Historical" connectNulls />
                <Line type="monotone" dataKey="forecast" stroke="#f97316" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 2 }} name="Forecast" connectNulls />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 md:gap-4">
        ...
        <div className="md:col-span-2 glass-card p-4 md:p-5">
          ...
          {loading ? <SkeletonTable rows={4} /> : filteredForecasts.length === 0 ? (
            <p className="text-center text-orange-400 text-sm py-10">No forecasts found.</p>
          ) : (
              ...
              {filteredForecasts.map((f) => (
                <div key={f.id} onClick={() => handleSelectForecast(f)}
                  className={`p-3 rounded-xl border cursor-pointer transition-all duration-150
                    ${selected?.id === f.id ? "border-orange-400 bg-primary-50 shadow-orange-glow" : "border-orange-100 hover:border-orange-300 hover:bg-primary-50/50"}
                    ${f.status !== "completed" ? "opacity-70 cursor-default" : ""}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-orange-900 truncate">{f.name}</p>
                      <p className="text-xs text-orange-400 font-mono mt-0.5">{f.model_type}</p>
                    </div>
                  ...
                  {f.accuracy_score && (
                    <div className="mt-1.5 flex gap-3 text-xs text-orange-500">
                      <span>R²: <strong className="text-orange-700">{(f.accuracy_score * 100).toFixed(1)}%</strong></span>
        ...
        <div className="hidden md:block md:col-span-3 glass-card p-5">
          <h2 className="section-title mb-4 flex items-center gap-2">
            <MdAutoGraph className="text-orange-500" />
          ...
          {selected?.status === "completed" && chartData.length > 0 ? (
            <div>
              <div className="flex flex-wrap gap-2 mb-4">
                {selected.accuracy_score && (
                  <div className="bg-primary-50 rounded-lg px-3 py-1.5 text-xs">
                    ...
                    <span className="font-bold text-orange-800 ml-1.5">{(selected.accuracy_score * 100).toFixed(1)}%</span>
                  </div>
                )}
                {selected.mae && (
                  <div className="bg-primary-50 rounded-lg px-3 py-1.5 text-xs">
                    <span className="text-orange-500">MAE</span>
                    <span className="font-bold text-orange-800 ml-1.5">{selected.mae.toFixed(3)}</span>
                  </div>
                )}
                {selected.rmse && (
                  <div className="bg-primary-50 rounded-lg px-3 py-1.5 text-xs">
                    <span className="text-orange-500">RMSE</span>
                    <span className="font-bold text-orange-800 ml-1.5">{selected.rmse.toFixed(3)}</span>
                  </div>
                )}
              </div>
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#ffedd5" />
                  <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#fb923c" }} interval="preserveStartEnd" />
                  <YAxis tick={{ fontSize: 10, fill: "#fb923c" }} />
                  <Tooltip contentStyle={{ background: "#f0fdf4", border: "1px solid #fed7aa", borderRadius: "12px", fontSize: "12px" }} />
                  <Legend wrapperStyle={{ fontSize: "12px" }} />
                  <ReferenceLine x={chartData.find((d) => d.type === "forecast")?.date} stroke="#f97316" strokeDasharray="4 4" />
                  <Line type="monotone" dataKey="historical" stroke="#ea580c" strokeWidth={2} dot={false} name="Historical" connectNulls />
                  <Line type="monotone" dataKey="forecast" stroke="#f97316" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 3 }} name="Forecast" connectNulls />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ) : selected?.status === "running" ? (
            <div className="h-64 flex flex-col items-center justify-center gap-3 text-orange-500">
              <div className="w-10 h-10 border-4 border-orange-100 border-t-primary-500 rounded-full animate-spin" />
              <p className="text-sm">Training… click 🔄 to refresh.</p>
            </div>
          ) : (
            <div className="h-64 flex flex-col items-center justify-center text-orange-300">
              <MdAutoGraph className="text-5xl mb-3 text-orange-200" />
