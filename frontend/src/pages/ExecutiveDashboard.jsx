...
const orangeS = ["#f97316", "#ea580c", "#fb923c", "#ea580c", "#fb923c", "#ea580c"];
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; margin: 40px; color: #ea580c; background: #f0fdf4; }
    h1 { color: #ea580c; border-bottom: 2px solid #f97316; padding-bottom: 10px; }
    h2 { color: #ea580c; margin-top: 30px; }
    .kpi-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin: 20px 0; }
    .kpi-card { background: white; border: 1px solid #fed7aa; border-radius: 12px; padding: 16px; }
    .kpi-card .label { font-size: 11px; color: #ea580c; text-transform: uppercase; letter-spacing: 0.5px; }
    .kpi-card .value { font-size: 24px; font-weight: bold; color: #ea580c; margin-top: 4px; }
    table { width: 100%; border-collapse: collapse; margin-top: 12px; }
    th { background: #ea580c; color: white; padding: 10px; text-align: left; font-size: 13px; }
    td { padding: 9px 10px; border-bottom: 1px solid #ffedd5; font-size: 13px; }
    tr:nth-child(even) td { background: #f0fdf4; }
    .badge { display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: bold; }
    .badge-good { background: #ffedd5; color: #ea580c; }
    .rec-item { background: white; border-left: 4px solid #f97316; padding: 10px 14px; margin: 8px 0; border-radius: 4px; font-size: 13px; }
    .footer { margin-top: 40px; padding-top: 12px; border-top: 1px solid #fed7aa; font-size: 11px; color: #ea580c; }
  </style>
</head>
<body>
  <h1>📊 ${report.title}</h1>
  <p style="color:#ea580c;font-size:13px">Generated: ${content.generated_at} &nbsp;|&nbsp; Period: ${period}</p>
  ...
  <div class="footer">
    ForecastNova Enterprise v5.0 &nbsp;|&nbsp; Confidential — For Internal Use Only
  </div>
</body>
</html>`;
    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-start justify-between gap-3 pt-2 md:pt-0">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button onClick={fetchData} className="btn-secondary p-2 text-orange-600"><MdRefresh className="text-lg" /></button>
        ...
        {loading ? [...Array(4)].map((_, i) => <SkeletonCard key={i} />) : kpis.map(({ label, value, icon: Icon, sub }) => (
          <div key={label} className="stat-card">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs font-medium text-orange-500 uppercase tracking-wider">{label}</p>
                <p className="font-display text-xl md:text-2xl font-bold mt-1" style={{ color: "var(--text)" }}>{value}</p>
                <p className="text-xs text-orange-400 mt-0.5">{sub}</p>
              </div>
              <div className="w-8 h-8 md:w-9 md:h-9 bg-primary-100 dark:bg-primary-900 rounded-xl flex items-center justify-center flex-shrink-0">
                <Icon className="text-orange-600 dark:text-orange-400 text-base md:text-lg" />
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4">
        <div className="glass-card p-4 md:p-5 lg:col-span-2">
          ...
          ) : <p className="text-sm text-center py-8 text-orange-400">No revenue data available yet.</p>}
        </div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={data.model_performance} layout="vertical">
                ...
                <Bar dataKey="avg_accuracy" radius={[0, 4, 4, 0]}>
                  {data.model_performance.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : <p className="text-sm text-center py-8 text-orange-400">Complete forecasts to see model performance.</p>}
        </div>
      </div>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={data.top_products.slice(0, 5)} layout="vertical">
                ...
                <Bar dataKey="revenue" radius={[0, 4, 4, 0]}>
                  {data.top_products.slice(0, 5).map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : <p className="text-sm text-center py-8 text-orange-400">No product data yet.</p>}
        </div>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={data.region_performance} dataKey="revenue" nameKey="region" cx="50%" cy="50%" outerRadius={65}
                  label={({ region }) => region}>
                  {data.region_performance.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
          ) : <p className="text-sm text-center py-8 text-orange-400">No region data yet.</p>}
        </div>
      </div>
        ...
        <div className="glass-card p-4 md:p-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-orange-100 dark:border-orange-800">
              ...
              <tbody>
                {data.recent_forecasts.map((f) => (
                  <tr key={f.id} className="border-b border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30">
                    <td className="table-td font-medium">{f.name}</td>
                    <td className="table-td font-mono text-xs">{f.model}</td>
                    <td className="table-td">{f.accuracy ? <span className="font-bold text-orange-600">{f.accuracy}%</span> : "—"}</td>
                    <td className="table-td"><span className={f.status === "completed" ? "badge-success" : "badge-info"}>{f.status}</span></td>
                    <td className="table-td text-orange-400">{f.created_at}</td>
                  </tr>
                ))}
              </tbody>
            </table>
