      ...
      {showForm && (
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2"><MdNotificationsActive className="text-orange-500" /> Configure Alert</h2>
      ...
      {tab === "configs" && (
        <div className="glass-card p-4 md:p-5 animate-fade-in">
          ...
          {loading ? <SkeletonTable /> : configs.length === 0 ? (
            <div className="text-center py-10"><MdNotificationsActive className="text-4xl text-orange-200 mx-auto mb-3" /><p className="text-sm" style={{ color:"var(--text-muted)" }}>No alerts configured yet.</p></div>
          ) : (
                ...
                <div key={c.id} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2 flex-wrap">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-2 mt-0.5 flex-wrap">
                      <span className="badge bg-primary-100 dark:bg-primary-800 text-orange-700 dark:text-orange-300 text-xs">{c.alert_type.replace(/_/g," ")}</span>
                      {c.threshold_value && <span className="text-xs text-orange-400">{c.threshold_operator} {c.threshold_value}</span>}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => handleToggle(c.id)} className="text-orange-500 hover:text-orange-700 text-sm px-2 py-1 rounded-lg bg-primary-100 dark:bg-primary-800">
              ...
              {logs.map((l) => (
                <div key={l.id} className={`flex items-start justify-between p-3 rounded-xl gap-2 ${!l.is_read ? "bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800" : "bg-primary-50 dark:bg-primary-900/40"}`}>
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-orange-400 mt-0.5">{l.message}</p>
                    <p className="text-xs text-orange-400 mt-0.5">{l.created_at?.slice(0,16).replace("T"," ")}</p>
                  </div>
                  {!l.is_read && (
                    <button onClick={() => handleMarkRead(l.id)} className="text-orange-500 hover:text-orange-700 flex-shrink-0">
