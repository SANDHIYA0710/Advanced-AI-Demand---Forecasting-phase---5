  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="glass-card p-4 md:p-6">
        <h2 className="section-title mb-4 flex items-center gap-2"><MdUpload className="text-orange-500" /> Upload Dataset</h2>
        <div className="space-y-3 md:space-y-4">
          ...
          <div {...getRootProps()}
            className={`border-2 border-dashed rounded-xl p-6 md:p-8 text-center cursor-pointer transition-all duration-200
              ${isDragActive ? "border-orange-400 bg-primary-50 dark:bg-primary-900/50" : "border-orange-200 dark:border-orange-800 hover:border-orange-400 hover:bg-primary-50/50 dark:hover:bg-primary-900/30"}
              ${file ? "border-orange-400 bg-primary-50 dark:bg-primary-900/50" : ""}`}>
            <input {...getInputProps()} />
            {file ? (
              <div className="flex items-center justify-center gap-3">
                <MdCheckCircle className="text-orange-500 text-xl md:text-2xl flex-shrink-0" />
                <div className="text-left min-w-0">
                  <p className="font-medium text-sm truncate" style={{ color: "var(--text)" }}>{file.name}</p>
                  <p className="text-xs text-orange-400">{(file.size / 1024).toFixed(1)} KB</p>
                </div>
                <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); }} className="ml-2 text-orange-400 hover:text-red-400 flex-shrink-0"><MdClose /></button>
              </div>
            ) : (
              <div>
                <MdUpload className="text-orange-300 text-3xl md:text-4xl mx-auto mb-2" />
                <p className="text-orange-600 dark:text-orange-400 font-medium text-sm">{isDragActive ? "Drop here" : "Tap or drag to select"}</p>
                <p className="text-orange-400 text-xs mt-1">CSV, XLSX, or XLS</p>
              </div>
            )}
          </div>
      ...
      <div className="glass-card p-4 md:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          <h2 className="section-title flex items-center gap-2"><MdStorage className="text-orange-500" /> Your Datasets</h2>
          <div className="flex gap-2 sm:ml-auto">
            <div className="relative flex-1 sm:flex-none">
              <MdSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-orange-400 text-sm" />
          ...
          <span className="text-xs text-orange-400">{total} total</span>
        </div>
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {datasets.map((ds) => (
                    <tr key={ds.id} className="border-b border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30 transition-colors">
                      <td className="table-td font-medium">{ds.name}</td>
                      <td className="table-td text-xs text-orange-400 font-mono">{ds.filename}</td>
                      <td className="table-td">{ds.rows_count?.toLocaleString()}</td>
                      <td className="table-td"><StatusBadge status={ds.status} /></td>
                      <td className="table-td text-orange-400">{ds.created_at?.slice(0,10)}</td>
                      <td className="table-td">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handlePreview(ds.id)} className="text-orange-500 hover:text-orange-700"><MdPreview className="text-lg" /></button>
                ...
                <div key={ds.id} className="bg-primary-50 dark:bg-primary-900/40 rounded-xl p-3 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-orange-400 font-mono truncate">{ds.filename}</p>
                    </div>
                    <StatusBadge status={ds.status} />
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex gap-3 text-xs text-orange-500">
      ...
      {(preview || previewLoading) && (
        <div className="glass-card p-4 md:p-5 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setPreview(null)} className="text-orange-400 hover:text-orange-600"><MdClose className="text-xl" /></button>
          </div>
          {previewLoading ? (
            <div className="h-32 flex items-center justify-center">
              <div className="w-6 h-6 border-2 border-orange-200 border-t-primary-600 rounded-full animate-spin" />
            </div>
          ) : (
            <div>
              <div className="flex gap-3 mb-3 text-xs text-orange-500 flex-wrap">
                <span>Shape: <strong style={{ color: "var(--text)" }}>{preview.shape?.rows} × {preview.shape?.cols}</strong></span>
              </div>
              <div className="overflow-x-auto rounded-xl border border-orange-100 dark:border-orange-800">
                <table className="w-full text-xs">
                  <thead className="bg-primary-50 dark:bg-primary-900">
                    <tr>{preview.columns?.map((col) => (
                      <th key={col} className="px-2 md:px-3 py-2 text-left font-semibold text-orange-600 dark:text-orange-400 whitespace-nowrap">{col}</th>
                    ))}</tr>
                  </thead>
                  <tbody>
                    {preview.preview?.map((row, i) => (
                      <tr key={i} className="border-t border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30">
