    ...
    <div className="space-y-4 md:space-y-6">
        ...
        {INTEGRATION_TYPES.map(({ value, label, desc }) => (
          <div key={value} className="glass-card p-3 md:p-4">
            <div className="w-8 h-8 bg-primary-100 dark:bg-primary-900 rounded-lg flex items-center justify-center mb-2">
              <MdIntegrationInstructions className="text-orange-600 dark:text-orange-400 text-base" />
            </div>
            <p className="font-semibold text-xs md:text-sm" style={{ color:"var(--text)" }}>{label}</p>
            <p className="text-xs text-orange-400 mt-0.5 hidden sm:block">{desc}</p>
          </div>
        ))}
      </div>
      ...
      {showForm && (
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2"><MdIntegrationInstructions className="text-orange-500" /> New Integration</h2>
            <button onClick={() => setShowForm(false)} className="text-orange-400 hover:text-orange-600"><MdClose /></button>
          </div>
      ...
      {selectedLogs && (
        <div className="glass-card p-4 md:p-5 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2"><MdHistory className="text-orange-500" /> Webhook Logs</h2>
            <button onClick={() => setSelectedLogs(null)} className="text-orange-400 hover:text-orange-600"><MdClose /></button>
          </div>
          {logsLoading ? <p className="text-sm text-orange-400">Loading…</p> : (
            <div className="space-y-2">
              {selectedLogs.logs?.length === 0 ? <p className="text-sm text-orange-400">No logs yet.</p> : (
                selectedLogs.logs.map((l, i) => (
                  <div key={i} className="flex items-center justify-between p-2.5 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2">
                    <span className="text-xs font-mono text-orange-500">{l.event_type}</span>
                    <div className="flex items-center gap-2">
                      <span className={l.success ? "badge-success" : "badge-error"}>{l.response_status || "—"}</span>
                      {l.success ? <MdCheckCircle className="text-orange-500" /> : <MdError className="text-red-400" />}
                      <span className="text-xs text-orange-400">{l.created_at?.slice(0,16).replace("T"," ")}</span>
                    </div>
                  </div>
                ))
              )}
      ...
      <div className="glass-card p-4 md:p-5">
        ...
        {loading ? <SkeletonTable /> : integrations.length === 0 ? (
          <div className="text-center py-10">
            <MdIntegrationInstructions className="text-4xl text-orange-200 mx-auto mb-3" />
              ...
              <div key={i.id} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2 flex-wrap">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex gap-2 mt-0.5 flex-wrap">
                    <span className="badge bg-primary-100 dark:bg-primary-800 text-orange-700 dark:text-orange-300 text-xs">{i.integration_type}</span>
                    <span className="text-xs text-orange-400 truncate max-w-xs">{i.endpoint_url}</span>
                  </div>
                  <p className="text-xs text-orange-400 mt-0.5">{i.trigger_count} triggers • Last: {i.last_triggered?.slice(0,10) || "Never"}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => handleViewLogs(i.id)} className="text-orange-500 hover:text-orange-700"><MdHistory className="text-lg" /></button>
                  <button onClick={() => handleToggle(i.id)} className="text-xs px-2 py-1.5 rounded-lg bg-primary-100 dark:bg-primary-800 text-orange-700 dark:text-orange-300">
