  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {showForm && (
        <div className="glass-card p-4 md:p-6 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2"><MdSchedule className="text-orange-500" /> Configure Schedule</h2>
      ...
      <div className="glass-card p-4 md:p-5">
        <h2 className="section-title mb-4 flex items-center gap-2"><MdSchedule className="text-orange-500" /> Active Schedules</h2>
        {loading ? <SkeletonTable /> : schedules.length === 0 ? (
          <div className="text-center py-12">
            <MdSchedule className="text-4xl text-orange-200 mx-auto mb-3" />
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {schedules.map((s) => (
                    <tr key={s.id} className="border-b border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30">
                      <td className="table-td font-medium">{s.name}</td>
                      <td className="table-td text-xs text-orange-400">#{s.dataset_id}</td>
                      <td className="table-td"><span className="font-mono text-xs bg-primary-100 dark:bg-primary-900 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded-lg">{s.model_type?.replace(/_/g," ")}</span></td>
                      <td className="table-td capitalize">{s.interval}</td>
                      <td className="table-td">{s.run_count}</td>
                      <td className="table-td text-xs text-orange-400">{s.next_run?.slice(0,16).replace("T"," ") || "—"}</td>
                      ...
                      <td className="table-td">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleRunNow(s.id)} className="text-orange-500 hover:text-orange-700" title="Run now"><MdFlashOn className="text-lg" /></button>
                          <button onClick={() => handleToggle(s.id)} className="text-orange-500 hover:text-orange-700" title={s.is_active ? "Pause" : "Resume"}>
                ...
                <div key={s.id} className="bg-primary-50 dark:bg-primary-900/40 rounded-xl p-3 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-orange-400 font-mono">{s.model_type?.replace(/_/g," ")} • {s.interval}</p>
                    </div>
