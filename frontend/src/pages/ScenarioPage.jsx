      ...
      <div className="flex items-start justify-between gap-3 pt-2 md:pt-0">
        <div>
          <h1 className="page-title flex items-center gap-2">
            <MdScience className="text-orange-500" /> What-If Scenario Analysis
          </h1>
                ...
                {varLabels.map(({ key, label, min, max, step }) => (
                  <div key={key} className="bg-primary-50 dark:bg-primary-900/40 rounded-xl p-3">
                    <div className="flex justify-between items-center mb-1.5">
                      ...
                      <span className="text-xs font-bold text-orange-600">{form.variables[key]}</span>
                    </div>
        ...
        <div className="glass-card p-4 md:p-5 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <h2 className="section-title flex items-center gap-2">
              <MdCompare className="text-orange-500" /> Scenario Comparison
            </h2>
            <button onClick={() => setCompareResult(null)} className="text-orange-400 hover:text-orange-600">
              <MdClose />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {compareResult.scenarios.map((s, i) => (
              <div key={s.id} className="bg-primary-50 dark:bg-primary-900/40 rounded-xl p-3 border-l-4"
                ...
                <p className={`text-xs mt-0.5 ${s.change_pct >= 0 ? "text-orange-500" : "text-red-400"}`}>
          ...
          {chartData.length > 0 ? (
            ...
            <p className="text-sm text-orange-400 text-center py-6">No prediction data to chart.</p>
          )}
        </div>
      )}
      ...
      <div className="glass-card p-4 md:p-5">
        ...
        {loading ? <SkeletonChart /> : scenarios.length === 0 ? (
          <div className="text-center py-10">
            <MdScience className="text-4xl text-orange-200 mx-auto mb-3" />
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {selected.includes(s.id) && (
                        <span className="badge bg-orange-500 text-white">✓ Selected</span>
                      )}
                    </div>
                    {s.variables && (
                      <div className="flex gap-3 mt-1.5 flex-wrap text-xs text-orange-400">
                    ...
                    {s.results && (
                      <div className="flex gap-4 mt-1.5 text-xs flex-wrap">
                        ...
                        <span className={s.results.change_pct >= 0 ? "text-orange-600" : "text-red-500"}>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    {!s.is_saved && s.status === "completed" && (
                      <button onClick={() => handleSave(s.id)}
                        className="text-orange-500 hover:text-orange-700 p-1" title="Save scenario">
