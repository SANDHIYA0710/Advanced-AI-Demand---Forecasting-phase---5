  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        <h1 className="page-title flex items-center gap-2"><MdAutoGraph className="text-orange-500" /> Forecast Accuracy Center</h1>
          ...
          <>
            {[
              ...
              <div key={label} className="stat-card">
                <p className="text-xs font-medium text-orange-500 uppercase">{label}</p>
        ...
        {loading ? <SkeletonChart /> : data?.accuracy_trend?.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={data.accuracy_trend}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="accuracy" stroke="#f97316" strokeWidth={2} dot={{ r: 2 }} name="R² Accuracy %" />
              <Line type="monotone" dataKey="mae" stroke="#3b82f6" strokeWidth={1.5} dot={false} name="MAE" />
            </LineChart>
          </ResponsiveContainer>
        ) : <p className="text-sm text-center py-8 text-orange-400">No completed forecasts yet.</p>}
      </div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={data.model_comparison}>
                ...
                <Bar dataKey="avg_r2" name="Avg R² %" radius={[4,4,0,0]}>
                  {data.model_comparison.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : <p className="text-sm text-center py-8 text-orange-400">Complete forecasts to compare models.</p>}
        </div>
                ...
                <div key={m.model} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-orange-400">{m.count} forecasts</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-orange-600">{m.avg_r2}%</p>
                    <p className="text-xs text-orange-400">MAE: {m.avg_mae}</p>
                  </div>
                </div>
              ))}
              {(!data?.model_comparison || data.model_comparison.length === 0) && (
                <p className="text-sm text-center py-4 text-orange-400">No model data yet.</p>
              )}
            </div>
          )}
        </div>
