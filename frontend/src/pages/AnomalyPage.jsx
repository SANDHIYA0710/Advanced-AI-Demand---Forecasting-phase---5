  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        ...
        <p className="text-orange-500 text-xs md:text-sm mt-1">Detect unusual sales patterns using IQR & Z-Score analysis</p>
      </div>

      {/* Config form */}
      <div className="glass-card p-4 md:p-5">
        <h2 className="section-title mb-4 flex items-center gap-2"><MdSearch className="text-orange-500" /> Detection Config</h2>
      ...
      {result && !loading && (
        <div className="space-y-4 animate-slide-up">
          ...
          <div className="glass-card p-4 md:p-5">
            ...
            <p className="text-sm text-orange-600">{result.summary}</p>
          </div>
                  ...
                  <tbody>
                    {result.anomalies.map((a, i) => (
                      <tr key={i} className="border-b border-orange-50 hover:bg-primary-50/50">
                        ...
                        <td className="table-td font-bold text-orange-900">{a.value}</td>
            ...
            <div className="glass-card p-4 md:p-5">
              ...
              <div className="flex gap-4 flex-wrap text-sm">
                <div className="bg-primary-50 rounded-xl px-3 py-2">
                  Peak Month: <strong className="text-orange-800">{result.seasonal_insights.peak_month}</strong>
                </div>
            ...
            {history.map((h) => (
              <div key={h.id} className="flex items-center justify-between p-3 bg-primary-50 rounded-xl gap-2">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-orange-900">Dataset #{h.dataset_id} — {h.target_column}</p>
                  <p className="text-xs text-orange-400 truncate">{h.summary}</p>
                </div>
