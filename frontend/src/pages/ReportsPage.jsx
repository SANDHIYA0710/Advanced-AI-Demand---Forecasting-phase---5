  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        ...
        <p className="text-orange-500 text-xs md:text-sm mt-1">Download reports and get AI-generated insights</p>
      </div>
        ...
        {[
          ...
          <div key={label} className="glass-card p-3 md:p-5 flex items-center gap-2 md:gap-3">
            <div className="w-8 h-8 md:w-12 md:h-12 bg-primary-100 rounded-xl flex items-center justify-center flex-shrink-0">
              <Icon className="text-orange-600 text-base md:text-2xl" />
            </div>
            <div className="min-w-0">
              <p className="font-semibold text-orange-900 text-xs md:text-sm">{label}</p>
              <p className="text-orange-400 text-xs mt-0.5 hidden sm:block">{sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {showInsights && (
        <div className="glass-card p-4 md:p-5 animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowInsights(false)} className="text-orange-400 hover:text-orange-600"><MdClose /></button>
          </div>
          {insightsLoading ? (
            ...
            <div className="space-y-3">
              <p className="text-sm text-orange-600">{insights.summary}</p>
              {insights.insights?.map((ins, i) => (
                <div key={i} className="flex items-start gap-3 p-3 bg-primary-50 rounded-xl">
                  ...
                  <p className="text-sm text-orange-700">{ins.message}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="glass-card p-4 md:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-4">
          <h2 className="section-title flex items-center gap-2"><MdAssessment className="text-orange-500" /> Completed Forecasts</h2>
        ...
        {loading ? <SkeletonTable /> : filtered.length === 0 ? (
          <div className="text-center py-10 md:py-14">
            <MdAssessment className="text-4xl md:text-5xl text-orange-200 mx-auto mb-3" />
            <p className="text-orange-400 text-sm">No completed forecasts found.</p>
          </div>
        ) : (
          <>
            {/* Desktop table */}
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {filtered.map((f) => (
                    <tr key={f.id} className="border-b border-orange-50 hover:bg-primary-50/50 transition-colors">
                      <td className="table-td font-semibold">{f.name}</td>
                      <td className="table-td"><span className="font-mono text-xs bg-primary-100 text-orange-700 px-2 py-0.5 rounded-lg">{f.model_type?.replace("_"," ")}</span></td>
                      <td className="table-td">{f.periods}</td>
                      <td className="table-td">{f.accuracy_score ? <span className={`font-bold ${f.accuracy_score > 0.8 ? "text-orange-600" : f.accuracy_score > 0.6 ? "text-yellow-600" : "text-red-500"}`}>{(f.accuracy_score * 100).toFixed(1)}%</span> : "—"}</td>
                      ...
                      <td className="table-td text-orange-400">{f.created_at?.slice(0,10)}</td>
                      <td className="table-td">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => handleDownload(f.id,"excel")} disabled={downloading[`${f.id}-excel`]} className="flex items-center gap-1 btn-secondary text-xs px-2 py-1.5">
                            {downloading[`${f.id}-excel`] ? <div className="w-3 h-3 border border-orange-400 border-t-primary-700 rounded-full animate-spin" /> : <TbFileSpreadsheet />} XLSX
                          </button>
              ...
              {filtered.map((f) => (
                <div key={f.id} className="bg-primary-50 rounded-xl p-3 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-orange-900 text-sm truncate">{f.name}</p>
                      <span className="font-mono text-xs bg-primary-100 text-orange-700 px-2 py-0.5 rounded-lg inline-block mt-1">{f.model_type?.replace("_"," ")}</span>
                    </div>
                    {f.accuracy_score && <span className={`text-sm font-bold flex-shrink-0 ${f.accuracy_score > 0.8 ? "text-orange-600" : f.accuracy_score > 0.6 ? "text-yellow-600" : "text-red-500"}`}>{(f.accuracy_score * 100).toFixed(1)}%</span>}
                  </div>
