      ...
      <div className="hidden md:flex w-1/2 bg-primary-800 dark:bg-primary-950 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative flex items-center gap-3">
          <div className="w-10 h-10 bg-primary-400 rounded-xl flex items-center justify-center">
            <MdAutoGraph className="text-orange-900 text-xl" />
          </div>
          <span className="font-display font-bold text-white text-xl">ForecastNova</span>
        </div>
        <div className="relative">
          <h2 className="font-display text-4xl font-bold text-white leading-tight mb-4">
            Data Into Decisions..<br /><span className="text-orange-300">Confidence Into Growth..</span>
          </h2>
          <p className="text-orange-200 text-base leading-relaxed max-w-sm">
            Empowering businesses with intelligent forecasting and predictive analytics
          </p>
          <div className="mt-8 grid grid-cols-3 gap-4">
            {[["95%","Accuracy"],["10×","Faster"],["∞","Scalable"]].map(([val,label]) => (
              <div key={label} className="bg-[#a63d14]/50 rounded-xl p-3 text-center">
                <div className="font-display text-2xl font-bold text-orange-300">{val}</div>
                <div className="text-xs text-orange-400 mt-0.5">{label}</div>
              </div>
            ))}
          </div>
        </div>
        <p className="relative text-orange-500 text-xs">© 2025 ForecastNova. All rights reserved.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 py-10 min-h-screen md:min-h-0">
        <div className="w-full max-w-md animate-slide-up">
          <div className="flex items-center gap-2 mb-8 md:hidden">
            <div className="w-9 h-9 bg-orange-500 rounded-xl flex items-center justify-center shadow-orange-glow">
              <MdAutoGraph className="text-white text-lg" />
            </div>
            <span className="font-display font-bold text-xl" style={{ color: "var(--text)" }}>ForecastNova</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-orange-400 hover:text-orange-600"
