...
const orangeS = ["#f97316","#ea580c","#fb923c","#ea580c"];
    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-start justify-between gap-3 pt-2 md:pt-0">
        <div>
          ...
          <p className="text-orange-500 text-xs md:text-sm mt-1">API activity, performance, and forecast history</p>
        </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                {[
                  ...
                  <div key={label} className="stat-card">
                    ...
                    <p className="font-display text-xl font-bold text-orange-900 mt-1">{value}</p>
                  </div>
                ))}
              </div>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={performance.slow_endpoints?.slice(0, 6)} layout="vertical">
                    ...
                    <Bar dataKey="avg_ms" radius={[0,4,4,0]}>
                      {performance.slow_endpoints?.slice(0,6).map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {logs.map((l) => (
                      <tr key={l.id} className="border-b border-orange-50 hover:bg-primary-50/50">
                        <td className="table-td font-mono text-xs">{l.action}</td>
                        <td className="table-td"><span className="badge bg-primary-100 text-orange-700">{l.method}</span></td>
                        <td className="table-td text-xs text-orange-400 truncate max-w-32">{l.endpoint}</td>
                        ...
                        <td className="table-td text-xs text-orange-400">{l.ip_address}</td>
                        <td className="table-td text-xs text-orange-400">{l.created_at?.slice(0,16).replace("T"," ")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {logs.map((l) => (
                  <div key={l.id} className="bg-primary-50 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-mono text-xs text-orange-700">{l.action}</span>
                      <span className={l.status_code < 400 ? "badge-success" : "badge-error"}>{l.status_code}</span>
                    </div>
                    <p className="text-xs text-orange-400 truncate">{l.endpoint}</p>
                    <p className="text-xs text-orange-400 mt-0.5">{l.response_time_ms}ms • {l.created_at?.slice(0,16).replace("T"," ")}</p>
                  </div>
                ))}
              </div>
            </>
            ...
            <>
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {history.map((f) => (
                      <tr key={f.id} className="border-b border-orange-50 hover:bg-primary-50/50">
                        ...
                        <td className="table-td">{f.accuracy_score ? <span className="font-bold text-orange-700">{f.accuracy_score}%</span> : "—"}</td>
                        ...
                        <td className="table-td text-xs text-orange-400">{f.created_at?.slice(0,10)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {history.map((f) => (
                  <div key={f.id} className="bg-primary-50 rounded-xl p-3 space-y-1">
                    <div className="flex items-center justify-between">
                      <p className="font-medium text-orange-900 text-sm truncate pr-2">{f.name}</p>
                      <span className={f.status === "completed" ? "badge-success" : f.status === "error" ? "badge-error" : "badge-info"}>{f.status}</span>
                    </div>
                    <p className="text-xs text-orange-400 font-mono">{f.model_type} • {f.target_column} • {f.periods} periods</p>
                    {f.accuracy_score && <p className="text-xs text-orange-600">Accuracy: <strong>{f.accuracy_score}%</strong></p>}
                  </div>
                ))}
              </div>
            </>
