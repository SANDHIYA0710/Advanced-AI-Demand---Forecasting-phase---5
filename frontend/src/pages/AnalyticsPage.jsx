  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="pt-2 md:pt-0">
        ...
        <p className="text-orange-500 text-xs md:text-sm mt-1">Deep insights into your demand patterns</p>
      </div>
        ...
        {tabs.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => { setTab(key); setData(null); }}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all whitespace-nowrap flex-shrink-0
              ${tab === key ? "bg-white text-orange-900 shadow-sm" : "text-orange-600 hover:text-orange-800"}`}>
...
function RegionAnalytics({ data }) {
  const orangeS = ["#f97316","#ea580c","#fb923c","#ea580c","#fb923c"];
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="glass-card p-4 md:p-5">
        <h2 className="section-title mb-4">Sales by Region</h2>
        <div className="mb-3 flex gap-3 text-xs text-orange-500 flex-wrap">
          <span>Top Region: <strong className="text-orange-800">{data.top_region}</strong></span>
          <span>Total: <strong className="text-orange-800">${data.total_value?.toLocaleString()}</strong></span>
        </div>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={data.regions}>
            ...
            <Bar dataKey="total" radius={[4,4,0,0]}>
              {data.regions?.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.regions?.map((r, i) => (
                <tr key={i} className="border-b border-orange-50 hover:bg-primary-50/50">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="glass-card p-4 md:p-5">
        ...
        <div className="mb-2 text-xs text-orange-500">Top: <strong className="text-orange-800">{data.top_category}</strong></div>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data.categories?.slice(0, 8)} layout="vertical">
            ...
            <Bar dataKey="total" radius={[0,4,4,0]}>
              {data.categories?.slice(0,8).map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      <div className="glass-card p-4 md:p-5">
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto scrollbar-thin">
          {data.categories?.map((c, i) => (
            <div key={i} className="flex items-center justify-between py-1.5 border-b border-orange-50">
              <span className="text-sm text-orange-900">{c.product}</span>
      ...
      <div className="glass-card p-4 md:p-5 md:col-span-2">
        ...
        <div className="flex gap-3 mb-4 flex-wrap text-xs">
          <div className="bg-primary-50 rounded-lg px-3 py-1.5">
            Avg Monthly: <strong className="text-orange-800">${data.avg_monthly_revenue?.toLocaleString()}</strong>
          </div>
          <div className={`rounded-lg px-3 py-1.5 ${data.growth_trend === "positive" ? "bg-primary-50 text-orange-700" : "bg-red-50 text-red-700"}`}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={combined}>
            ...
            <Tooltip />
            <Legend />
            <Line type="monotone" dataKey="revenue" stroke="#ea580c" strokeWidth={2} dot={false} name="Revenue" connectNulls />
          </LineChart>
        </ResponsiveContainer>
      </div>
      <div className="glass-card p-4 md:p-5 md:col-span-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {data.predictions?.map((p, i) => (
            <div key={i} className="bg-primary-50 rounded-xl p-3 text-center">
              <p className="text-xs text-orange-500">{p.month}</p>
              <p className="font-display text-lg font-bold text-orange-900 mt-1">${p.predicted_revenue?.toLocaleString()}</p>
              <p className={`text-xs mt-0.5 ${p.growth_rate_pct >= 0 ? "text-orange-600" : "text-red-500"}`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="glass-card p-4 md:p-5">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <p className="text-sm text-orange-600">{data.risk_message}</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={label} className="bg-primary-50 rounded-xl p-3">
              ...
              <p className="font-bold text-orange-900 mt-0.5">{value}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {data.insights?.map((insight, i) => (
            <div key={i} className="flex items-start gap-2 p-2.5 bg-primary-50 rounded-xl">
              <MdWarning className="text-orange-500 flex-shrink-0 mt-0.5" />
              <p className="text-xs text-orange-700">{insight}</p>
            </div>
          ))}
        </div>
      </div>
