  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex items-center gap-3 pt-2 md:pt-0">
        <div className="w-9 h-9 md:w-10 md:h-10 bg-orange-500 rounded-xl flex items-center justify-center flex-shrink-0">
              ...
              <div key={label} className="stat-card">
                <div className="flex items-start justify-between">
                  ...
                  <div className="w-8 h-8 bg-primary-100 dark:bg-primary-900 rounded-xl flex items-center justify-center">
                    <Icon className="text-orange-600 dark:text-orange-400 text-base" />
                  </div>
                </div>
              </div>
            ))}
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={stats.model_stats}>
                    ...
                    <Bar dataKey="avg_accuracy" radius={[4,4,0,0]}>
                      {stats.model_stats.map((_, i) => <Cell key={i} fill={orangeS[i % orangeS.length]} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            <div className="glass-card p-4 md:p-5">
              ...
              <div className="space-y-2">
                {stats?.recent_users?.map((u) => (
                  <div key={u.id} className="flex items-center justify-between py-2 border-b border-orange-100 dark:border-orange-800 gap-2">
                    ...
                    <span className="text-xs text-orange-400 flex-shrink-0">{u.created_at}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="glass-card p-4 md:p-5 animate-fade-in">
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            <div className="relative flex-1 min-w-0">
              <MdSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-orange-400" />
            ...
            <span className="text-sm text-orange-500 flex-shrink-0">{userTotal} users</span>
          </div>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-orange-50 dark:border-orange-900 hover:bg-primary-50/50 dark:hover:bg-primary-900/30">
                    <td className="table-td">
                      <p className="font-medium" style={{ color: "var(--text)" }}>{u.username}</p>
                      <p className="text-xs text-orange-400">{u.email}</p>
                    </td>
                    <td className="table-td">
                    ...
                    <td className="table-td text-orange-400">{u.created_at}</td>
                    <td className="table-td">
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleToggleActive(u.id)} className="text-orange-500 hover:text-orange-700" title={u.is_active ? "Deactivate" : "Activate"}>
            ...
            {users.map((u) => (
              <div key={u.id} className="bg-primary-50 dark:bg-primary-900/40 rounded-xl p-3 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => handleToggleActive(u.id)} className="text-orange-500 hover:text-orange-700">
            ...
            {stats?.recent_forecasts?.map((f) => (
              <div key={f.id} className="flex items-center justify-between p-3 bg-primary-50 dark:bg-primary-900/40 rounded-xl gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-orange-400">Model: {f.model} • User #{f.owner_id}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <span className={f.status === "completed" ? "badge-success" : f.status === "error" ? "badge-error" : "badge-info"}>{f.status}</span>
                  <p className="text-xs text-orange-400 mt-1">{f.created_at}</p>
                </div>
              </div>
            ))}
          </div>
