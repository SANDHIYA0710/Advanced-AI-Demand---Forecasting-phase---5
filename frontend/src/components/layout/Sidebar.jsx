...
export default function Sidebar() {
  ...
  const SidebarContent = () => (
    <>
      <div className="px-4 py-4 border-b border-orange-100 dark:border-orange-900 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-orange-500 rounded-lg flex items-center justify-center shadow-orange-glow">
            <MdAutoGraph className="text-white text-lg" />
          </div>
          <div>
            <span className="font-display font-bold text-sm leading-none" style={{ color:"var(--text)" }}>ForecastNova</span>
            <p className="text-[10px] text-orange-500 mt-0.5">Enterprise v5.0</p>
          </div>
        </div>
        <button onClick={closeMobile} className="md:hidden text-orange-400 hover:text-orange-700"><MdClose className="text-xl" /></button>
      </div>

      <nav className="flex-1 px-3 py-3 overflow-y-auto scrollbar-thin">
        {navGroups.map(({ label, items }) => (
          <div key={label} className="mb-3">
            <p className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-orange-400">{label}</p>
        ...
        {user?.is_admin && (
          <div className="pt-2 mt-1 border-t border-orange-100 dark:border-orange-900">
      ...
      <div className="px-3 pb-4 border-t border-orange-100 dark:border-orange-900 pt-3">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-primary-50 dark:bg-primary-900/50 mb-2">
          <div className="w-7 h-7 bg-primary-200 dark:bg-primary-800 rounded-full flex items-center justify-center flex-shrink-0">
            <MdPerson className="text-orange-700 dark:text-orange-300 text-sm" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold truncate" style={{ color:"var(--text)" }}>{user?.full_name || user?.username}</p>
            <p className="text-[10px] text-orange-500 truncate capitalize">{user?.is_admin ? "👑 Super Admin" : user?.role || "analyst"}</p>
          </div>
        </div>
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)}
        className="md:hidden fixed top-4 left-4 z-50 w-9 h-9 bg-white dark:bg-primary-950 border border-orange-200 dark:border-orange-800 rounded-xl flex items-center justify-center shadow-glass">
        <MdMenu className="text-orange-700 dark:text-orange-300 text-xl" />
      </button>
      ...
      <aside className={`md:hidden fixed top-0 left-0 h-full w-60 backdrop-blur-md border-r flex flex-col z-50 transition-transform duration-300
        bg-white/95 dark:bg-primary-950/95 border-orange-100 dark:border-orange-900
        ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <SidebarContent />
      </aside>
      <aside className="hidden md:flex w-56 min-h-screen backdrop-blur-md border-r flex-col
        bg-white/80 dark:bg-primary-950/80 border-orange-100 dark:border-orange-900 transition-colors duration-300">
        <SidebarContent />
      </aside>
    </>
  );
