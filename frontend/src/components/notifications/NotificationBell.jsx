...
const typeColors = { success:"border-l-4 border-orange-400 bg-primary-50 dark:bg-primary-900/50", error:"border-l-4 border-red-400 bg-red-50 dark:bg-red-900/30", warning:"border-l-4 border-yellow-400 bg-yellow-50 dark:bg-yellow-900/30", info:"border-l-4 border-blue-400 bg-blue-50 dark:bg-blue-900/30" };
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={handleOpen} className="relative p-2 rounded-xl transition-colors hover:bg-primary-100 dark:hover:bg-primary-900">
        {unread > 0 ? <MdNotifications className="text-xl text-orange-600 dark:text-orange-400" /> : <MdNotificationsNone className="text-xl text-orange-600 dark:text-orange-400" />}
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-10 z-50 w-[calc(100vw-2rem)] sm:w-80 max-w-sm glass-card shadow-xl overflow-hidden animate-scale-in">
            <div className="flex items-center justify-between px-4 py-3 border-b border-orange-100 dark:border-orange-800">
              <h3 className="font-semibold text-sm" style={{ color:"var(--text)" }}>Notifications</h3>
              {unread > 0 && <button onClick={markAllRead} className="flex items-center gap-1 text-xs text-orange-600 dark:text-orange-400 hover:text-orange-800"><MdDoneAll /> Mark all read</button>}
            </div>
            <div className="max-h-72 overflow-y-auto scrollbar-thin">
              {loading ? <div className="p-4 space-y-2">{[...Array(3)].map((_, i) => <div key={i} className="h-14 bg-primary-50 dark:bg-primary-900 rounded-lg animate-pulse" />)}</div>
               : notifications.length === 0 ? <div className="p-8 text-center text-orange-400 text-sm"><MdNotificationsNone className="text-3xl mx-auto mb-2" />No notifications</div>
               : notifications.map((n) => (
                <div key={n.id} onClick={() => !n.is_read && markRead(n.id)} className={`px-4 py-3 border-b border-orange-50 dark:border-orange-900 cursor-pointer ${typeColors[n.type] || ""} ${!n.is_read ? "font-medium" : "opacity-60"}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0"><p className="text-xs font-semibold truncate" style={{ color:"var(--text)" }}>{n.title}</p><p className="text-xs text-orange-600 dark:text-orange-400 mt-0.5 line-clamp-2">{n.message}</p><p className="text-[10px] text-orange-400 mt-1">{n.created_at?.slice(0,16).replace("T"," ")}</p></div>
                    <button onClick={(e) => { e.stopPropagation(); deleteNotif(n.id); }} className="text-orange-300 hover:text-red-400 flex-shrink-0"><MdClose className="text-sm" /></button>
                  </div>
                </div>
              ))}
            </div>
