...
def send_forecast_complete_email(to_email: str, forecast_name: str, accuracy: float = None):
    ...
    html = f"""
    <h2 style="color:#ea580c">Forecast Completed ✅</h2>
    <p>Your forecast <strong>{forecast_name}</strong> has been completed{acc_text}.</p>
    <p><a href="http://localhost:3000/forecast" style="background:#f97316;color:white;padding:10px 20px;border-radius:8px;text-decoration:none">View Forecast</a></p>
    """
...
def send_alert_email(to_email: str, alert_name: str, message: str):
    ...
    <p>{message}</p>
    <p><a href="http://localhost:3000/dashboard" style="background:#f97316;color:white;padding:10px 20px;border-radius:8px;text-decoration:none">View Dashboard</a></p>
    """
    send_email(to_email, subject, body, html)
